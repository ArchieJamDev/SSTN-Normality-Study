...
- [ ] Chequeo de factibilidad Fleishman/GLD sobre esos 11 puntos objetivo; decidir método único. **API ya confirmada contra la documentación real** (`SimMultiCorrData::find_constants(method="Fleishman"|"Polynomial", skews=, skurts=, fifths=, sixths=)` → `$valid`; `skurts` es curtosis EN EXCESO; `gld::fit.fkml(x, method="ML")` ajusta directo a datos crudos, no a un objetivo (asimetría,curtosis)). Piloto `R/04b_pilot_feasibility.R` + job `feasibility-pilot` del workflow (depende de `extract-subscales`) ya escrito y subido — pendiente de correr para ver resultados reales antes de escribir la versión final de `04_moments_and_feasibility.R`.
