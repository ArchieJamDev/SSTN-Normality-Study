...
- [ ] Chequeo de factibilidad Fleishman/GLD sobre esos 11 puntos objetivo; decidir método único. Piloto `R/04b_pilot_feasibility.R` corrido — ver sección 12: **Fleishman infeasible para las 11 subescalas** (confirmado), y el chequeo Polynomial/GLD crasheó por un bug de manejo de retorno de `find_constants()` (ya corregido con el helper `get_valid()`). Pendiente: re-correr el job `feasibility-pilot` con la corrección para obtener los resultados reales de Polynomial y GLD antes de escribir la versión final de `04_moments_and_feasibility.R`.
